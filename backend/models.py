...
class Society(Base):
    ...
    documents     = relationship("Document",     back_populates="society")
...
class Flat(Base):
    ...
    flat_type        = Column(String(20))          # 1BHK, 2BHK, 3BHK
    area_sqft        = Column(Float)
    maintenance_rate = Column(Float)               # per sqft or fixed amount
...
class User(Base):
    ...
    emergency_contact_mobile = Column(String(15))
    id_proof_type   = Column(String(50))           # aadhaar, pan, passport
    id_proof_number = Column(String(100))
    vehicle_numbers = Column(JSON, default=list)   # list of vehicle plate numbers
    ...
    complaints    = relationship("Complaint",   back_populates="created_by", foreign_keys="Complaint.created_by_id")
...
class Visitor(Base):
    ...
    host_name       = Column(String(200))          # name of resident being visited
    expected_arrival = Column(DateTime(timezone=True), nullable=True)
    created_at      = Column(DateTime(timezone=True), server_default=func.now())

    society = relationship("Society", back_populates="visitors")
    flat    = relationship("Flat",    back_populates="visitors", foreign_keys=[flat_id])

    __table_args__ = (
        Index("ix_visitors_society_date", "society_id", "created_at"),
    )
...
class Budget(Base):
    ...
    __table_args__ = (
        UniqueConstraint("society_id", "financial_year", "category", name="uq_budget_fy_cat"),
    )
...
class AppSetting(Base):
    ...
    value          = Column(Text)
    category       = Column(String(50))   # db, email, sms, whatsapp, general, billing, security
